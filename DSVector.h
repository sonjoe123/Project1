...
public:
  ...
  // reserve more space: to change the size of the array
  // đầu tiên nó sẽ so sánh cái capacity mình muốn với cái nó đang hiện có nếu nó nhỏ hơn thì ko sao nhma nếu nó lớn hơn thì"
  // đầu tiên nó sẽ tạo 1 pointer point to the an array of new object that has the new capicity, then it will copy the old value over to the new array
  // then it will delete/ deallocate the old one (the old array)
  // reassign the pointer to the new array and reassign the current capacity
  // tại vì cái new array only exist in this func only thats why we need to assign it back to the main pointer: Oject
  ...
  // resize the vector
  // give the reserve func the new size that the user wants
